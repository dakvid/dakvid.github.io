...
description: If you live in New Zealand you would have been hard pressed not to notice that we had local body elections recently. It seems that all anyone can talk about is the turnout. Is it all bad news?
...
```{r get_turnout_data}
if(!file.exists("data/summary_turnout.csv") |
   !file.exists("data/council_turnout.csv")) {
  lg_tables <- 
  ...
  summary_turnout <- 
    ...
    write_csv(summary_turnout, "data/summary_turnout.csv")
    ...
    write_csv(council_turnout, "data/council_turnout.csv")
} else {
...
```{r turnout_summary, fig.height=2, preview=TRUE}
nto <- 
  summary_turnout %>% 
  spread(Year, Turnout) %>% 
...
plot_ly(nto,
  ...
  add_markers(x = ~`2010`, y = ~Group, name = "2010", color = I("yellow"),
              hoverinfo = "text",
              text = ~paste0(`2010`, "% turnout in ", Group, " councils in 2010")) %>% 
  add_markers(x = ~`2013`, y = ~Group, name = "2013", color = I("blue"),
              hoverinfo = "text",
              text = ~paste0(`2013`, "% turnout in ", Group, " councils in 2013")) %>% 
  add_markers(x = ~`2016`, y = ~Group, name = "2016", color = I("red"),
              hoverinfo = "text",
              text = ~paste0(`2016`, "% turnout in ", Group, " councils in 2016")) %>% 
  layout(
...
plot_ly(cto,
  ...
  add_markers(x = ~`2010`, y = ~Council, name = "2010", color = I("yellow"),
              hoverinfo = "text",
              text = ~paste0(`2010`, "% turnout in ", Council, " in 2010")) %>% 
...
[by council](data/council_turnout.csv).
